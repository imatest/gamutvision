...
!define VER_DISPLAY     "1.4"  ; Must be the same as handles.version in gamutvision.m.
!define SOURCE          "..\gamutvision\installation\"
!define GVSOURCE        "..\gamutvision\"  ; added by NLK (Not sure what I'm doing)
!define MATLAB          "..\Matlab\"
!define MATLAB_LIBS_PATH "toolbox\matlab\iofun\private\"
!define MATLAB_LIBS_URL  "http://www.gamutvision.com/packages/Imatest-lib.exe"
; !define MATLAB_LIBS_URL  "http://www.imatest.com/packages/Imatest-lib.exe"  ; fails
...
; Configuration
   ...
   OutFile              "..\gamutvision\installer\Gamutvision-${VER_DISPLAY}.exe"    ; Output File
...
Section "Gamutvision (required)" SecCore
  ...
  file ${SOURCE}${MATLAB_LIBS_PATH}jpeg_depth.dll  ; Redundant: not in Matlab library  ORIGINAL!
  file ${SOURCE}${MATLAB_LIBS_PATH}imjpg8.dll
  file ${SOURCE}${MATLAB_LIBS_PATH}rjpg8c.dll
  file ${SOURCE}${MATLAB_LIBS_PATH}ibrowse.dll
  file ${SOURCE}${MATLAB_LIBS_PATH}sortcellchar.dll
  file ${SOURCE}${MATLAB_LIBS_PATH}uigetfolder_win32.dll
  file ${SOURCE}${MATLAB_LIBS_PATH}uigetfiles.dll
  ...
  FileWrite $2 "echo Loading Gamutvision...$\r$\n"
  FileWrite $2 "gamutvision.exe diag$\r$\n"
  ...
  CreateDirectory "$APPDATA\Imatest"
...
libdone:  ; Place AddToPath(s) here so they are executed AFTER Library installation.
   ...
   ; The following lines removed Oct 2016 after jpeg_depth issue. Try again?
   ; CopyFiles /FILESONLY ${SOURCE}jpeg_depth.dll ${GVSOURCE}${MATLAB_LIBS_PATH}jpeg_depth.dll
   ; CopyFiles /FILESONLY ${SOURCE}imjpg8.dll  ${GVSOURCE}${MATLAB_LIBS_PATH}imjpg8.dll
   ; CopyFiles /FILESONLY ${SOURCE}rjpg8c.dll  ${GVSOURCE}${MATLAB_LIBS_PATH}rjpg8c.dll
   file /oname=${MATLAB_LIBS_PATH}jpeg_depth.dll   ${SOURCE}jpeg_depth.dll
   file /oname=${MATLAB_LIBS_PATH}imjpg8.dll       ${SOURCE}imjpg8.dll
   file /oname=${MATLAB_LIBS_PATH}rjpg8c.dll       ${SOURCE}rjpg8c.dll
   file /oname=${MATLAB_LIBS_PATH}ibrowse.dll      ${SOURCE}ibrowse.dll
